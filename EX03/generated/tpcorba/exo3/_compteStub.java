	...
	implements tpcorba.exo3.compte
{
	...
	public int nombre_operations()
	{
		...
		if(! this._is_local())
		{
			...
			try
			{
				org.omg.CORBA.portable.OutputStream _os = _request("_get_nombre_operations",true);
				_is = _invoke(_os);
				return _is.read_long();
			}
			catch( org.omg.CORBA.portable.RemarshalException _rx ){}
		...
		{
		org.omg.CORBA.portable.ServantObject _so = _servant_preinvoke( "_get_nombre_operations", _opsClass);
		if( _so == null )
			throw new org.omg.CORBA.UNKNOWN("local invocations not supported!");
			compteOperations _localServant = (compteOperations)_so.servant;
			int _result;
		try
		{
			_result = _localServant.nombre_operations();
		}
		finally
		{
			_servant_postinvoke(_so);
